...
| `src/jira.js` | `transitionIssue`, `addComment`, `getIssue`, `createWorklog` |
...
5. Emit preview via preview() — terminal log + SLACK_PREVIEW_CHANNEL post
6. Call doTransition
...
`createWorklog(issueKey, opts?)` defaults: `timeSpentSeconds = 3600`, `description = 'Implement based on solution design & implementation plan, self-review, self-test'`, `started = <today in Asia/Bangkok>T12:00:00.000+0700`. The Jira comment field is prefixed with `[Generated by personal bot]`. Errors are swallowed (logged, but the caller continues).
